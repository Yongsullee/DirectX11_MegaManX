...
void Sprite::Update(D3DXMATRIX & V, D3DXMATRIX & P)
{
	...
	D3DXMatrixScaling(&S, scale.x * textureSize.x , scale.y * textureSize.y , 1);
	...
	// Sprite Bouding Box의 위치 정보 Update
	SetRectTop();
	SetRectBottom();
	SetRectLeft();
	SetRectRight();
...
/*
*	Sprite Bouding Box의 위치 정보
*/
void Sprite::SetRectTop()
{
	float yScale = Scale().y * TextureSize().y * 0.5f;

	rectTop = Position().y + yScale;
}

void Sprite::SetRectBottom()
{
	float yScale = Scale().y * TextureSize().y * 0.5f;

	rectBottom = Position().y - yScale;
}

void Sprite::SetRectLeft()
{
	float xScale = Scale().x * TextureSize().x * 0.5f;

	rectLeft = Position().x - xScale;
}

void Sprite::SetRectRight()
{
	float xScale = Scale().x * TextureSize().x * 0.5f;

	rectRight = Position().x + xScale;
}
